...
- Flat layout (deliberate — single popup page): `manifest.json`, `popup.html`, `popup.css`, `icons/` sit at the package root, entries in `src/`. `build.mjs` copies statics from the package root, not `src/`; the pear credit icon is resolved from `@browser-plugins/assets` at build time (no local copy).
- Extension display name: **Begone** (manifest `name` and `action.default_title`); package name: `@begone/extension`.
- No inline styles or CSS-in-JS — popup styling lives in `popup.css` with block-element classNames (`popup__*`); visibility is toggled with the `hidden` attribute. Popup ends with the monorepo-standard pearpages credit footer (`.popup__credit`, 16px pear icon + "Made by pearpages"). Design tokens are `--be-*` custom properties at the top of `popup.css`, keyed to the app icon's flat blue `#2d9cdb` (NOT the promo site's red — user decision 2026-07-22); dark mode via `prefers-color-scheme`. The header mark is an inline currentColor SVG (✕ in a rounded square), not the PNG icon.
...
- Pear credit icon deduped monorepo-wide: `build.mjs` now copies it from `@browser-plugins/assets` (via `import.meta.resolve`) instead of a package-root copy; the site's `public/` copies (pear, favicon, `plugins/`) are gone — site-kit imports them from the assets package.

### 2026-07-22 (later) — popup restyle: icon-blue theme + dark mode
- Full `popup.css` rewrite after user feedback ("header does not look good, plugin too plain"): new `--be-*` token set keyed to the icon's flat blue `#2d9cdb` (user explicitly rejected red for the popup), cool neutrals, dark block via `prefers-color-scheme` (begone is no longer the light-only outlier). Header banner is now the blue gradient with an inline currentColor SVG ✕ mark (rounded square — mirrors the icon) instead of the PNG-on-white-chip. Polish pass modeled on focaccia: pill mono input with accent focus ring, pill Add button with `0 2px 0` hard shadow + active press, selector rows as bordered 10px cards with mono accent-colored `<code>`, "banished" hover motif (blue strikethrough on the selector), 24px circular delete that inverts to accent on hover, danger-chip error, tokenized credit, `:focus-visible` + reduced-motion guard, fixed 320px width.
- Verified: build green, popup screenshot-checked light + dark + row-hover via `--load-extension` (selectors added through the real form → storage.sync round-trip works).
- **Note:** the promo site still uses the red "vanishing act" palette — popup (icon-blue) and site (red) now diverge; realigning the site is an open follow-up for the user to decide.
