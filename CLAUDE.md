...
- **A11y conventions (all sites, WCAG AA)**: `pnpm sites:a11y` (`scripts/a11y-check.mjs`) runs axe over all six built pages light+dark and must stay clean — run it after touching site markup or theme tokens. Rules the check encodes: (1) decorative text-built heroes are `<figure role="img" aria-label="…">` — role="img" makes descendants presentational; the script excludes those subtrees from axe's contrast rule (false positives on decorative art). (2) Layout provides the skip link (`.sk-skip` → `#main`), global `:focus-visible` outline (colored by `--sk-accent`) and a global `prefers-reduced-motion` kill switch — every page's `<main>` must carry `id="main"` and wrap everything between hero and footer. (3) Site `theme.css` muted-ink and CTA tokens deliberately diverge from the extension palettes for contrast (each carries a comment); CTA pattern is darker-bg+white-ink in light mode, bright-accent-bg+dark-ink in dark mode via `--*-cta-*` tokens, hovers darken (light) / lighten (dark) — never reuse the raw accent under white text.
...
### 2026-07-23 — a11y pass over all six sites (WCAG AA, axe-clean)
- Full audit + fixes; conventions recorded in the Deployment section's a11y bullet. Highlights: the three text-built heroes (begone fake DOM, headerforge HTTP exchange, hopchase redirect chain) got `role="img"` so AT reads only the aria-label (headerforge's two invalid inner `<figcaption>`s became `<p>`s); Layout gained skip link + global `:focus-visible` + reduced-motion CSS; every page's `<main id="main">` now wraps features+InstallSteps+OtherPlugins (the old `main.features` grid is a `div.features` inside it); OtherPlugins grid is a real `<ul>`, its "→" is aria-hidden, InstallSteps' `<pre>` is keyboard-scrollable (`tabindex=0` region); home card names are `<h2>`s (structural spans → divs/ps for validity).
- Contrast: muted-ink tokens darkened on all six sites (e.g. begone `#888`→`#6f6f6f`, home `#85816f`→`#706c5a`); new `--*-cta-*` tokens on begone/cookiejar/headerforge/hopchase (light: darker brand bg + white; dark: bright accent bg + dark ink); focaccia's CTA hover now darkens (`--red-dark`); headerforge direction colors got AA `--hf-*-text` variants (dark request-text `#7d9fff` — `#5b87ff` fails on the danger tint); hopchase 2xx/3xx pills darkened. `generate-og.mjs` THEMES muted values updated to match + cards regenerated.
- New root devDeps `@playwright/test` + `@axe-core/playwright`, script `sites:a11y`. Verified: typecheck + `sites:build` green, axe clean 6 pages × light/dark, keyboard pass (Tab 1 = skip link, Tab 2 = CTA with visible ring), all six pages screenshot-checked light+dark — visuals effectively unchanged.
